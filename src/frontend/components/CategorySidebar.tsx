    ...
    categories.forEach(cat => {
      if (!menu[cat.slug]) {
        ...
        if (cat.sections && cat.sections.length > 0) {
          ...
          // Fallback logic
          const uniqueBrands = Array.from(new Set(catProducts.map(p => p.brand))).filter(Boolean);
  ...
  return (
    <div className="relative z-40 h-full">
      ...
      {activeMegaMenu && (
        <div 
          ref={flyoutRef}
          className="absolute left-full ml-4 w-[800px] xl:w-[900px] bg-card border border-border rounded-[var(--radius-outer)] shadow-[var(--shadow-xl)] overflow-hidden animate-in fade-in slide-in-from-left-4 duration-300 flex flex-col"
          style={{ 
            top: `${clampedTop}px`, 
