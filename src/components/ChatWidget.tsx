...
export const ChatWidget: React.FC = () => {
  ...
  const handleSend = async (textOverride?: string, attachments?: string[], skipAI: boolean = false) => {
    ...
    // 1. Send User Message
    await chatService.sendMessage(sessionId, text, "user", undefined, attachments);
    ...
    return (
      <div className="mt-2 space-y-2">
        {attachments.map((url, i) => {
          ...
          if (isImage) {
            return (
              <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="block">
                <img src={url} alt="Attachment" className="max-w-full rounded-lg border border-slate-200 shadow-sm hover:opacity-90 transition-opacity" referrerPolicy="no-referrer" />
              </a>
            );
          }
          ...
          if (isAudio) {
            return (
              <div key={i} className="bg-slate-100 p-2 rounded-lg flex items-center gap-2">
          ...
          return (
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer" 
              className="flex items-center gap-2 p-2 bg-slate-100 rounded-lg text-xs hover:bg-slate-200 transition-colors"
            >
  ...
  return (
    <motion.div 
      ...
      animate={position}
      ref={containerRef}
      className={`fixed ${isMobile ? 'bottom-24' : 'bottom-6'} right-6 z-[9999] flex flex-col items-end pointer-events-none`}
    ...
    >
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="mb-4 w-[calc(100vw-32px)] sm:w-[350px] md:w-[420px] pointer-events-auto"
          >
            <Card className="shadow-2xl border-primary/20 overflow-hidden flex flex-col h-[500px] sm:h-[600px] max-h-[85vh]">
              <CardHeader 
                className="bg-primary text-primary-foreground p-4 flex flex-row items-center justify-between shrink-0 cursor-grab active:cursor-grabbing touch-none"
                onPointerDown={(e) => dragControls.start(e)}
              >
              ...
              <CardContent className="p-0 flex-1 overflow-hidden flex flex-col bg-muted/30">
                      ...
                      <div key={msg.id || idx} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[85%] p-3 rounded-2xl text-sm shadow-sm ${
                          msg.sender === "user" 
                            ...
                            : msg.sender === "human"
                            ? "bg-orange-100 text-orange-900 rounded-tl-none border border-orange-200"
                            : "bg-card text-foreground rounded-tl-none border border-border"
                        }`}>
                          <div className="flex items-center gap-2 mb-1">
                            {msg.sender === "assistant" && <Bot className="h-3 w-3 opacity-50" />}
                            {msg.sender === "human" && <User className="h-3 w-3 opacity-50" />}
                            <span className="text-[9px] font-bold uppercase tracking-widest opacity-50">
                              {msg.sender === "user" ? "You" : msg.sender === "human" ? "Support Agent" : "FixItAll AI"}
                            </span>
                          </div>
                ...
                <div className="flex flex-col w-full gap-2">
                  <div className="flex w-full items-center gap-2">
                    ...
                    <Button 
                      variant="ghost" 
                      size="icon" 
                      className="h-9 w-9 text-muted-foreground hover:bg-slate-100 rounded-full"
                    ...
                    {isRecording ? (
                      ...
                      <div className="flex-1 relative">
                        <Input 
                          ...
                          className="bg-slate-100 border-none focus-visible:ring-2 focus-visible:ring-primary/20 rounded-2xl pr-10 h-10 text-sm"
                          disabled={isUploading}
                        />
                        <Button 
                          variant="ghost" 
                    ...
                    {!inputValue.trim() && !isRecording && !recordedAudio ? (
                      <Button 
                        variant="ghost"
                        size="icon" 
                        className="h-10 w-10 rounded-full hover:bg-slate-100 text-muted-foreground" 
      ...
      <motion.button
        ...
        className="h-14 w-14 rounded-full bg-primary text-primary-foreground shadow-2xl flex items-center justify-center hover:bg-primary/90 transition-all relative group pointer-events-auto cursor-grab active:cursor-grabbing touch-none"
      >
