...
const SalesReport = () => {
  ...
  const handleFilter = () => {
    ...
    if (dateFrom) {
      filtered = filtered.filter(o => (typeof o.createdAt.toDate === 'function' ? o.createdAt.toDate() : new Date(o.createdAt as string | number | Date)) >= new Date(dateFrom));
    }
    if (dateTo) {
      filtered = filtered.filter(o => (typeof o.createdAt.toDate === 'function' ? o.createdAt.toDate() : new Date(o.createdAt as string | number | Date)) <= new Date(dateTo));
    }
    if (orderStatus !== "all") {
  ...
  const totalAmount = filteredOrders.reduce((acc, o) => acc + o.totalAmount, 0);
                ...
                <tbody className="divide-y divide-border">
                  {filteredOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-accent/20 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-xs text-muted-foreground">{format(typeof order.createdAt.toDate === 'function' ? order.createdAt.toDate() : new Date(order.createdAt as string | number | Date), "MMM dd, yyyy")}</span>
                      </td>
                      <td className="px-6 py-4">
                        <div>
