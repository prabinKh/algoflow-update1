...
const CustomerReports = () => {
  ...
  const activeCustomers = customers.filter(c => c.visitCount > 5).length;
  const newCustomers = customers.filter(c => {
    const lastVisit = typeof c.lastVisit.toDate === 'function' ? c.lastVisit.toDate() : new Date(c.lastVisit as string | number | Date);
    const now = new Date();
        ...
        <div className="p-4 lg:p-8 max-w-7xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 gsap-reveal">
            {[
              ...
              { label: "New Customers", value: newCustomers, icon: UserPlus, color: "text-indigo-500" },
              { label: "Top Customers", value: customers.filter(c => getCustomerStats(c.email).totalSpent > 5000).length, icon: Star, color: "text-amber-500" },
            ].map((stat, i) => (
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-border">
                  {filteredCustomers.map((customer) => {
                    ...
                    return (
                      <tr key={customer.uid} className="hover:bg-accent/20 transition-colors group">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center text-sm font-bold">
                              {customer.displayName.charAt(0)}
                            </div>
                            <p className="text-sm font-bold truncate max-w-[150px]">{customer.displayName}</p>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <span className="text-xs font-bold">{stats.orderCount} orders</span>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                            <Calendar size={12} />
                            {format(typeof customer.lastVisit.toDate === 'function' ? customer.lastVisit.toDate() : new Date(customer.lastVisit as string | number | Date), "MMM dd, yyyy")}
                          </div>
                        </td>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            <div className="w-16 h-1.5 bg-accent rounded-full overflow-hidden">
                              <div 
                                ...
                                style={{ width: `${Math.min((customer.visitCount / 20) * 100, 100)}%` }}
                              />
                            </div>
