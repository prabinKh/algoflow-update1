...
const CATEGORY_ICONS_MAP: Record<string, string[]> = {
  ...
  "Glass & Lens (Optical/Eyewear)": [
    "Glasses", "Eye", "EyeOff", "Sun", "Search", "Sparkles", "ShieldCheck", "Percent", "Heart", "Activity"
  ],
  "Cafe (Coffee Shop / F&B E-commerce)": [
    "Coffee", "CupSoda", "GlassWater", "Cake", "Croissant", "Cookie", "IceCream", "Flame", 
    "Soup", "Utensils", "UtensilsCrossed", "Table", "Store"
  ],
  "Hotels (Hospitality / Booking Platform)": [
    "Hotel", "Bed", "Home", "Key", "Calendar", "MapPin", "Map", "Compass", "ShieldCheck", 
    "Coffee", "Luggage", "ConciergeBell", "Wifi", "Tv", "Waves"
  ],
  "Liquor Shop (Alcoholic Beverages)": [
    "Wine", "Beer", "GlassWater", "Martini", "Flame", "ShieldCheck", "Timer", "Percent"
  ]
};
...
const AddProduct = () => {
  ...
  const { user } = useAuth();
  const businessType = user?.company?.business_type || "Electronics";
  const presetIcons = CATEGORY_ICONS_MAP[businessType] || CATEGORY_ICONS_MAP["Electronics"];
  ...
  const getBusinessIcon = (type?: string) => {
    switch (type) {
      ...
      case "Beauty & Personal Care": return "💄";
      case "Grocery & Daily Essentials": return "🛒";
      case "Glass & Lens (Optical/Eyewear)": return "👓";
      case "Cafe (Coffee Shop / F&B E-commerce)": return "☕";
      case "Hotels (Hospitality / Booking Platform)": return "🏨";
      case "Liquor Shop (Alcoholic Beverages)": return "🍷";
      default: return "🏢";
    }
  };
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                  ...
                  {user?.company?.business_type && (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary/10 text-primary text-[11px] font-bold uppercase tracking-wider border border-primary/20 shadow-sm self-start sm:self-auto">
                      <span>{getBusinessIcon(user?.company?.business_type)}</span>
                      <span>{user?.company?.business_type}</span>
                    </span>
                  )}
                </div>
              </div>
