...
const AdminDashboard = () => {
  ...
  const activeRepairs = useMemo(() => repairTickets.filter(t => t.status !== "Completed" && t.status !== "Cancelled").length, [repairTickets]);
  
  // Low Stock Products
  const lowStockProducts = useMemo(() => productsList.filter(p => (p.stock || 0) < 10).slice(0, 5), [productsList]);
  ...
  const topProducts = useMemo(() => {
    return productsList
      ...
      .map(p => ({
        id: p.id,
        name: p.name,
        sales: orders.reduce((acc, o) => acc + (o.items || []).filter(item => item && (item.productId === p.id || item.id === p.id)).length, 0),
        image: p.image
      }))
        ...
        <div className="p-4 lg:p-8 max-w-7xl mx-auto space-y-8 w-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 gsap-reveal">
            {[
              ...
              { label: "Total Orders", value: orders.length, icon: ShoppingCart, color: "text-blue-500", trend: "+8.2%", isUp: true, bg: "bg-blue-500/5" },
              { label: "Total Customers", value: customers.length, icon: Users, color: "text-indigo-500", trend: "+5.1%", isUp: true, bg: "bg-indigo-500/5" },
